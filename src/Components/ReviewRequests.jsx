    ...
    return (
        <ul className="list bg-base-300 rounded-box shadow-md mt-24 flex flex-col gap-4 max-w-2/3 mx-auto">
            ...
            {myPendingRequests.length > 0 ? myPendingRequests.map((request) =>
                <UserList key={request._id} profileUrl={request.profileUrl} firstName={request.firstName} lastName={request.lastName} gender={request.gender} age={request.age} skills={request.skills} />) : <li className="p-4 pb-2 text-xs opacity-60 tracking-wide">No pending requests</li>}
